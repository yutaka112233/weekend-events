...
2. **以下をWeb検索・取得し、条件に合うイベントを集める**
   ...
   上記は子育て向けカテゴリのサイトだが、**そこに限定しない**。お祭り・花火大会・
   マルシェ・地域のお祝いごとなど、家族で見物・参加できる一般的なイベントも
   同じくらい重視して探す（横浜市観光情報サイトやハマイベは子どもカテゴリ以外の
   ページ・検索も確認する、「横浜 花火大会」「横浜 お祭り」のような一般検索も行う）。

3. **条件に合うものを8〜12件を目安に選び、`events.json` の `events` 配列を差し替える**
   - 遊び場・屋内施設・お祭り・花火大会・マルシェなど、種類が偏らないように
     バリエーションを意識する
